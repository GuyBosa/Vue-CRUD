...
It relies on the HTTP standard, which means it’s format-agonistic and you can use XML, JSON, HTML, etc. The client and server are independent. In other words, the REST protocol separates the data storage and the UI from the server. Lastly, it can be scaled quickly primarily due to the separation between the client and the server.
